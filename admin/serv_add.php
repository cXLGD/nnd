<?php
    ...
    if(isset($_POST['sub'])){
        ...
        // $upload_arr['serv_url'] = $_POST['url'];
        // $upload_arr['serv_time'] = time();
        $upload_arr['serv_img'] = $img;
        $upload_arr['serv_thumb'] = $thumb;


        // $sql = "INSERT INTO `nnd_servs` (`serv_name`,`serv_desc`,`serv_img`,`serv_time`,`serv_type`) VALUES ('$title','$content','$img','$time',$type)";
        $res = insert('nnd_service',$upload_arr);
        // pre($res);
        // die;


        if($res['code']==1){
            echo "<script>alert('添加成功！');window.location.href='serv_list.php'</script>";
            die;
        }else{
