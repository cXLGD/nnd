<?php
    ...
    if(isset($_GET['info_type']) && !empty($_GET['info_type']) && $_GET['info_type'] >0){
        $type = $_GET['info_type'];
          // pre($_GET);
          // die;
      // 查询记录总数
      $con = "WHERE info_type = {$type}";
      $count_arr = select_one('info','COUNT(*) AS `count`',$con);
      $count = $count_arr['count'];


	    $condition = "INNER JOIN `nnd_info_type` AS type ON nnd_info.info_type=type.info_type_id  WHERE info_type = {$type} LIMIT {$offset},{$limit}";
	    $info = select_all('info','*',$condition);
    }else{
      //查询总记录数
      $count_arr = select_one('info','COUNT(*) AS `count`');
      $count = $count_arr['count'];
      $condition = "INNER JOIN `nnd_info_type` AS type ON nnd_info.info_type=type.info_type_id LIMIT {$offset},{$limit}   ";
      $info = select_all('info','*',$condition);
    ...
    if(isset($_GET['del']) && $_GET['del']>0){
      ...
      if(isset($_GET['info_type'])){
        $type= "?info_type=".$_GET['info_type'];
      }else{
        $type="";
      }
       
      $res = del('nnd_info',"WHERE info_id= {$id}");
      if($res['code'] == 1){
    ...
    if(isset($_POST['idarr'])){
      ...
      // pre($id_str);die;
      $condition = "WHERE info_id IN ({$id_str})";

      $res = del('nnd_info',$condition);
	    if($res['code']==1){
		    echo "<script>alert('删除成功!');window.location.href='{$current_url}?info_type={$type}';</script>";
		    die;
	    }else{
		    echo "<script>alert('删除失败!');</script>";
	    }
	 ...
	 <div class="row">
        <div class="col-md-12">
			<div class="panel">
                ...
                <form action="info_list.php" method="post">
                <div class="panel-body">
                  ...
                  <table class="table table-striped table-bordered table-hover dataTable">
                      ...
                      <tr class="success">
                        ...
                        <td>
                            <img src="<?php echo $item['info_thumb']; ?>" width="150px" alt="">
                        </td>
                        <td><?php echo $item['info_type_name']; ?></td>
                        
                        <td><?php echo date("Y/m/d H:i",$item['info_time']); ?></td>
                        <!-- 操作 -->
                        <td>
		                      <div class="btn-group">
		                        <a href="info_edit.php?edit=<?php echo $item['info_id']; ?>" class="btn btn-default btn-gradient"><span class="glyphicons glyphicon-pencil"></span></a>
                            <a onclick="return confirm('确定要删除吗？');" 
                                href="<?php echo $url.'del='.$item['info_id'] ;?>" class="btn btn-default btn-gradient dropdown-toggle"><span class="glyphicons glyphicon-trash"></span></a>
		                      </div>
