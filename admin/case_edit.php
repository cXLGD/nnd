<?php
        ...
        if(isset($_GET['edit']) && $_GET['edit'] >0){
            ...
            $condition = "WHERE case_id = {$id} ";
            $cases = select_one('cases','*',$condition);
            // var_dump($cases);
            // die;
            // $old_img = $cases['case_img'];
        }
        if(isset($_POST['edit'])){
            $id = $_POST['case_id'];
            ...
            if($_FILES['pic1']['error'] == 0){
                ...
                // $upload_arr['case_time'] = time();
                $upload_arr['case_img'] = $img;
                $upload_arr['case_thumb'] = $thumb;
            ...
            // die;
            $condition = "WHERE case_id = {$id}";
            $res = update('nnd_cases',$upload_arr,$condition);
            if($res['code'] == 1){
                // die;
                                ...
                                <div class="col-md-7">
                                    <div class="form-group">
                                        <div class="input-group"><span class="input-group-addon">分类</span>
                                            <select name="type" id="standard-list1" class="form-control">
                                                <!-- <option>请选择</option> -->
                                                <?php foreach($case_type as $item){ ?>
                                                    <option value="<?php echo $item['case_type_id']; ?>" 
                                                    <?php if($item['case_type_id']== $cases['case_type']){echo 'selected="selected"';} ?>>
                                                        <?php echo $item['case_type_name1']; ?>
                                                    </option>
                                                <?php } ?>
                                            </select>
                                        </div>
                                    ...
                                    <div class="form-group">
                                        <div class="input-group"><span class="input-group-addon">名称</span>
                                            <input type="text" name="name" value="<?php $cases['case_name']; ?>"
                                                   class="form-control">
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <div class="fileinput fileinput-new" data-provides="fileinput"  id="exampleInputUpload">
                                            <input type="hidden" name="old_img" value="<?php echo $cases['case_img'];?>"/>
